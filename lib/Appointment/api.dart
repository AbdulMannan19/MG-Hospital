...
class AppointmentApiService {
  ...
  Future<List<Map<String, dynamic>>> getBranches() async {
    final response = await _supabase
        .from('hospitalbranch')
        .select('id, name');
    return (response as List).map((e) => {'id': e['id'], 'name': e['name']}).toList();
  }

  Future<List<Map<String, dynamic>>> getSpecializations(String branchName) async {
    ...
    final specRes = await _supabase
        ...
        .select('name,id')
        .in_('id', specIds);
    return (specRes as List).map((e) => {'id': e['id'], 'name': e['name']}).toList();
  }
  ...
  Future<List<String>> getTimeSlots(int branchId, int specializationId, String day) async {
